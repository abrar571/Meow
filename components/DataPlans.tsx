...
import { Wifi, Phone, Tv, Flame, Star, Zap, Sliders, ChevronDown, Coins, ArrowUpRight, Globe } from 'lucide-react';
...
const REDEEM_PACKAGES: DataPackage[] = [
  {
    id: 'r1',
    name: 'Mini Pack | 100 MB',
    dataAmountGB: 0.1,
    voiceMinutes: 0,
    priceBDT: 0,
    coinsEarned: 0,
...
const DataPlans: React.FC<DataPlansProps> = ({ buyPackage }) => {
  const [activeTab, setActiveTab] = useState<'packs' | 'mixer' | 'redeem'>('packs');
  ...
  return (
    <div className="animate-fade-in">
      <div className="flex flex-col md:flex-row justify-between items-center mb-8">
        ...
        <div className="bg-white/5 p-1 rounded-xl border border-white/10 shadow-sm flex mt-4 md:mt-0 flex-wrap gap-1">
          <button 
            onClick={() => setActiveTab('packs')}
            className={`px-4 md:px-6 py-2 rounded-lg font-bold transition-all text-sm md:text-base ${activeTab === 'packs' ? 'bg-white text-black shadow-md' : 'text-slate-400 hover:text-white hover:bg-white/5'}`}
          >
            Regular Packs
          </button>
          <button 
            onClick={() => setActiveTab('redeem')}
            className={`px-4 md:px-6 py-2 rounded-lg font-bold transition-all flex items-center gap-2 text-sm md:text-base ${activeTab === 'redeem' ? 'bg-gradient-to-r from-yellow-500 to-orange-500 text-white shadow-md' : 'text-slate-400 hover:text-white hover:bg-white/5'}`}
          >
            <Coins className="w-4 h-4" /> Use Coins
          </button>
          <button 
            onClick={() => setActiveTab('mixer')}
            className={`px-4 md:px-6 py-2 rounded-lg font-bold transition-all flex items-center gap-2 text-sm md:text-base ${activeTab === 'mixer' ? 'bg-gradient-to-r from-purple-500 to-pink-500 text-white shadow-md' : 'text-slate-400 hover:text-white hover:bg-white/5'}`}
          >
      ...
      {activeTab === 'redeem' && (
        <div className="grid grid-cols-1 gap-4 max-w-2xl mx-auto animate-fade-in">
           <div className="text-center mb-6">
             <div className="inline-flex items-center gap-2 bg-yellow-500/10 text-yellow-500 px-4 py-1 rounded-full text-sm font-bold border border-yellow-500/20 mb-2">
                <Coins className="w-4 h-4" /> Use Coin
             </div>
             <p className="text-slate-400 text-sm">Redeem your Ryze Coins for instant data packs.</p>
           </div>
           ...
           {REDEEM_PACKAGES.map((pkg) => (
             <div 
               ...
               className={`relative h-28 w-full bg-gradient-to-r ${pkg.color} rounded-2xl p-6 flex flex-col justify-between overflow-hidden shadow-lg cursor-pointer hover:scale-[1.02] transition-transform`}
             >
                ...
                <div className="relative z-10 flex justify-between items-start">
                  <h3 className="text-xl font-bold text-white flex items-center gap-2">
                    {pkg.name} <ArrowUpRight className="w-4 h-4 text-white/70" />
                  </h3>
                </div>

                <div className="relative z-10 flex justify-between items-end">
                   <div>
                     <div className="flex items-center gap-1.5 text-white/90 font-medium text-sm mb-1">
                        <Globe className="w-4 h-4" />
                        <span>{pkg.dataAmountGB} GB</span>
                     </div>
                     <span className="text-[10px] text-white/60 uppercase font-bold tracking-wider">Internet</span>
                   </div>
                   
                   <div className="text-right">
                      <div className="flex items-center justify-end gap-1.5 mb-1">
                         <span className="text-xl font-black text-white">{(pkg.priceCoins! >= 1000000 ? (pkg.priceCoins! / 1000000).toFixed(2) + 'M' : (pkg.priceCoins! / 1000).toFixed(0) + 'K')}</span>
                         <div className="w-6 h-6 rounded-full bg-black/20 flex items-center justify-center border border-white/20">
                            <Coins className="w-3.5 h-3.5 text-white" />
                         </div>
                      </div>
                      <span className="text-xs text-white/80 font-medium">{pkg.validityDays} Days</span>
                   </div>
                </div>
             </div>
           ))}
              ...
              <div className="p-4 space-y-3 -mt-6 relative z-20">
                ...
                <div className="bg-[#1a1a1e] rounded-xl p-4 border border-white/5">
                 ...
                 <div className="bg-[#1a1a1e] rounded-xl p-4 flex justify-between items-center border border-white/5">
                  ...
                  <span className="text-white font-bold text-lg">{pkg.coinsEarned.toLocaleString()}</span>
                </div>
